...
export default function UpdateCollection() {
    ...
    useEffect(() => {
        const fetchCollections = async () => {
            try {
                const res = await fetch(`${API_URL}/api/collection/all`);
                if (!res.ok) throw new Error("Error getting collections");
                const data = await res.json();
                setCollections(Array.isArray(data) ? data : []);
            } catch (err) {
                console.error("Error loading collections:", err);
                setError("The collections could not be loaded.");
            }
        };
    ...
    const handleSelectCollection = async (collectionId) => {
        ...
        try {
            const res = await fetch(`${API_URL}/api/collection/${collectionId}`);
            if (!res.ok) throw new Error("Error loading collection");
            const data = await res.json();
            ...
            setMessage("Error loading collection. Check console for details.");
        }
    };
    ...
    const handleSubmit = async () => {
        if (!selectedCollectionId) {
            setMessage("Please select a collection to update.");
            return;
        }
        ...
        try {
            const res = await fetch(`${API_URL}/api/collection/update`, {
                method: "PUT",
                headers: { "Content-Type": "application/json" },
            ...
            if (!res.ok) {
                ...
                throw new Error(text || "Error updating collection");
            }

            setMessage(`Collection "${name}" updated successfully!`);

            const refreshRes = await fetch(`${API_URL}/api/collection/all`);
            if (refreshRes.ok) {
            ...
            setMessage("Error updating collection. Check console for details.");
        }
    };
                    ...
                    {cards.map((card) => (
                        ...
                        >
                            ...
                            {selectedCards.some((c) => c.cardId === card.cardId) && (
                                ...
                                >
                                    x{selectedCards.find((c) => c.cardId === card.cardId)?.quantity}
                                </div>
                            )}
                        </div>
                    ))}
