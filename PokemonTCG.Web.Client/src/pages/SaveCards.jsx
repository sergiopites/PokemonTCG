...
export default function SaveCards() {
    ...
    const handleSave = async () => {
        ...
        setLogs([]);
        try {
            const r = await fetch(API_URL + "/api/card/addpokemoncards/progress", {
                method: "POST",
            ...
            while (true) {
                ...
                for (const line of lines) {
                    ...
                    try {
                        ...
                        if (evt.done) setMessage(ICO_OK + " Cards synced successfully!");
                        if (evt.error) setMessage(ICO_FAIL + " Error: " + evt.error);
                    } catch (pe) { /* ignore parse errors */ }
                }
            }
            if (!message) setMessage(ICO_OK + " Cards synced successfully!");
        } catch (e) {
            console.error(e);
